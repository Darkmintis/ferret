  ...
  group('CurlExporter', () {
    ...
    test('escapes single quotes in url', () {
      final curl = const CurlExporter().export(
        testEntry(
          'q',
          url: Uri.parse("https://example.com/path?q=it's"),
        ),
      );
  ...
  group('HarExporter', () {
    test('builds valid HAR 1.2 with sorted entries', () {
      ...
      final har = jsonDecode(
        const HarExporter().export([late, early]),
      ...
      expect(
        (entries[0] as Map)['startedDateTime']
            .toString()
            .startsWith('2026-01-01T10'),
        isTrue,
      );
    });
    ...
    test('pending calls use status 0', () {
      final har = jsonDecode(
        const HarExporter().export([testEntry('p', status: null)]),
      ) as Map<String, dynamic>;
      final entry =
    ...
    test('includes queryString and redacts sensitive headers', () {
      ...
      final cookie = responseHeaders
          .cast<Map<Object?, Object?>>()
          .firstWhere((h) => h['name'] == 'set-cookie');
      expect(cookie['value'], '***');
    ...
    test('empty session exports empty entries array', () {
      final har = jsonDecode(const HarExporter().export(const []))
          as Map<String, dynamic>;
